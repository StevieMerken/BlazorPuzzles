...
Instead of rolling our own service, we are going to take advantage of `Microsoft.Extensions.Hosting.BackgroundService`. This provides a nice base class for a background service. The code below is well-commented, so further explanation is not necessary:
...
Here's how to configure it in *Program.cs*:

```c#
builder.Services.AddHostedService<MyBackgroundTask>();
```

And that's it. You do not have to touch this in any Blazor Component or page. 